...
This is the next execution handoff. Pause Deploy integration and prove the data-production/training route first. Offline converters, two training configs, a subprocess memory guard, canonical image acquisition, portable labeling-run records and review overlays are implemented; real labeling, manual review and training have not been run. Do not confuse synthetic tests with model execution.
...
Outstanding execution work is deliberate: dataset/model acquisition, canonical image/overlay generation, annotation run records and manifest assembly, real human review, pinned-library parser check, actual inference/training and training-record writing. Lightweight conversion and memory-guard behavior are unit-tested independently of these external operations.
